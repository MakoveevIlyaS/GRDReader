...
PyObject* read_and_draw(PyObject*, PyObject* args) {
	...
	PyObject* colorZeroAlpha = PyList_GetItem(args, 2);
	...
	for (int y = 0; y < Ny; y++) {
		...
		for (int x = 0; x < Nx; x++) {
			 ...
			 //if (round(zMatrix[y][x] * 100) / 100 < 0.001) { PyList_SetItem(imageRow, x, colorZeroAlpha);  }

			if (zMatrix[y][x] <= Zmin ) { PyList_SetItem(imageRow, x, colorZeroAlpha);  }
			else
			{
				for (int level = 0; level < levels; level++) {
...
static PyModuleDef reader_module = {
	...
	"reader",                        
	"Provides fast function for read and build contours from .grd files",
	0,
	reader_methods
};
