...
class GRDReader(QMainWindow):
    ...
    def setMenu(self):
        ...
        openGRDAction2 = QAction("Открыть H.grd файл", self)
        openGRDAction2.setStatusTip('Открыть .grd файл')
        openGRDAction2.triggered.connect(self.openFileDialog_H)

        openGRDAction3 = QAction("Delete", self)
        openGRDAction3.setStatusTip('d')
        openGRDAction3.triggered.connect(self.Delete)

        openGRDAction4 = QAction("Return", self)
        openGRDAction4.setStatusTip('d')
        openGRDAction4.triggered.connect(self.Return)
        ...
        menu.addAction(openGRDAction2)
        menu.addAction(openGRDAction3)
        menu.addAction(openGRDAction4)
    ...
    def openFileDialog_H(self):
        ...
        self.matrix, self.img_h = self.readGRD_H(fname, 30)
        #self.map.setPhoto_h(self.img_h)
        self.map.setPhoto_new(self.img_h)
   
       #Add matrix of Relief 
    ...
    def readGRD(self, filename, levels):
        ...
        colorZeroAlpha = [0,0,0,0]
 

        matrix, imgMatrix = read_and_draw([filename, colorRange, colorZeroAlpha])
        ...
        #_pixmap =  QPixmap(QPixmap.fromImage(img))

        return matrix, QPixmap(QPixmap.fromImage(img))


    #Add matrix of Water (H_)

    def readGRD_H(self, filename, levels):
        
        white = Color("#00FFFF")
        blue = Color("blue")

        colorRange = list(white.range_to(blue, levels))
        colorRangeCopy = []
        colorZeroAlpha = []
        ...
        return matrix, _pixmap # QPixmap(QPixmap.fromImage(img))
